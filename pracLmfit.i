 ...
 *
 * History:
 *	$Id: lmfit.i,v 1.2 1997/07/28 08:26:25 eric Exp $
 *	$Log: lmfit.i,v $
...
func lmfit_Learn(f, x, &a, y, w, fit=, correl=, stdev=, gain=, tol=, deriv=, itmax=,  lambda=, eps=, monte_carlo=,verb=)
{
    local grad;
    /* Maybe subset of parameters to fit. */
    ...
    /* Other settings. */
    diagonal= indgen(1:nfit^2:nfit+1);	// Subscripts of diagonal elements
    if (is_void(lambda)) lambda= 1e-3;
    if (is_void(gain)) gain= 10.0;
    if (is_void(itmax)) itmax= 100;
    if (is_void(eps)) eps= 1e-6;	// sqrt(machine_precision)/100
	...
	if (deriv) {
		...
		anew(fit(i)) += inc(i);
		grad(,i)= (f(x,anew)-m)(*)/inc(i);
                // Added to be able to stop the fitting process when diverging ...
                if(checkStopCondition()) {
                  "ok je m'arrete bientot !!"
                  break;
                }
	    }
	    neval += nfit;
	...
	alpha *= gamma(,-) * gamma(-,);
        ...
        if(verb)
          write,format="chi2: %g\r", chi2;
          
	while (1) {
            alpha(diagonal)= 1.0 + lambda;
	    anew= a;
	    anew(fit) += gamma * LUsolve(alpha, beta);
	...
	niter++;
        ...
        if(checkStopCondition(1)) {
          write, "ok je m'arrete de suite !!!!!!!!";
          break;
        }
        if(verb){
          write,format="Learn in Progress... iteration num %d\r",niter;
          tracePupilles, a, 0, 0;
        }
	conv= 2.0*(chi2-chi2new)/(chi2+chi2new);
	if (conv <= tol)
	...
	alpha(diagonal)= 1.0;
	alpha= LUsolve(alpha);
	if (anyof((tmp1= alpha(diagonal)) < 0.0))
          if(verb)
	    write, format=warn+"%s\n", "found negative variance(s)";
	tmp1= sqrt(abs(tmp1));
	if (stdev) {
...
func checkStopCondition(stat)
/* DOCUMENT 
     checkStopCondition() allows to detect when stop condition is 'on'
     checkStopCondition(1) allows to detect when stop condition is 'on', and resets it to 'off'
     checkStopCondition(0) sets stop condition to 'off'
   SEE ALSO:
 */
{
  stopfile = "/tmp/abortNow";
  if( stat==0 ) {
      remove, stopfile;
  } else {
    fic = open(stopfile,"r",1);
    if(fic) {
      close,fic;
      if( stat==1 )
        remove, stopfile;
