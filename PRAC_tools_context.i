...
func determinesSlopesdisFromSlopestl(timetl,&pathtl,&voltstl,&MI,arcsec=)
 ...
 */
{
  ...
  returnSlopestl,timetl,pathtl,fake=1;
  slopestl = returnSlopestl(timetl,pathtl);
  ...
  if(dimsof(*ptr_volts(1))(1) == 0)
    return 0;
  //manages voltages
  voltstl = *ptr_volts(1);//54xNframes matrix in voltages
  voltsDM = 3276.8*(voltstl);//traduces volts in ADU
  data.rtc.volts = &voltsDM;
  ...
  MI = restorefits("mi",suffMI,pathMI);//72x54 matrix in pixel/ADU
  data.rtc.MI = &MI;
  //computes delay
  ...
  rawslopes = MI(,+)*vconv(+,);
  //flip of the WFS
  slopesv = mirror_SH7(rawslopes, data.wfs(data.its).sym);
  //adds to engaged slopes
  slopesdis(slrange(data.its),) -= slopesv;
  slopesdis = slopesdis(,3:-2);

  if(arcsec)
    for(i=1;i<=data.nwfs;i++) {
      slopesdis(slrange(i),) *= data.wfs(i).pixarc;
  }
  return slopesdis;
 ...
 */
{
  ...
  if(!fake){
    nwfs = numberof(tmp);
    slopestl = array(0., data.wfs(1).nssp, nwfs, dimsof(*tmp(1))(4));
  
    for(i=1;i<=nwfs;i++){
      slopestl(,i,) = (*tmp(i))(*,);
    }
    for(i=1;i<=nwfs;i++) {
      slopestl(,i,) = mirror_SH7( slopestl(,i,) , data.wfs(i).sym );
    ...
    if(arcsec)
      for(i=1;i<=nwfs;i++) {
        slopes_res(slrange(i),) *= data.wfs(i).pixarc;
      }
 ...
 */
{
  ...
  nwfs = numberof(tmp);
  slopesdis = array(0., data.wfs(1).nssp, nwfs, dimsof(*tmp(1))(4));
  
  for(i=1;i<=nwfs;i++) slopesdis(,i,) = (*tmp(i))(*,);
  for(i=1;i<=nwfs;i++) {
    slopesdis(,i,) = mirror_SH7( slopesdis(,i,) , data.wfs(i).sym );// remove WFS orientations for slopesdis
  }
  slopes_dis = slopesdis(*,);

  if(arcsec)
    for(i=1;i<=nwfs;i++) {
      slopes_dis(slrange(i),) *= data.wfs(i).pixarc;
  }
 ...
 */
{
  if(is_void(n)) n=sum(data.wfs.nssp);
  if(is_void(x))
    return 1:n;
  if( is_range(x) )
    x=indgen(x);
 ...
 */
{
  n0 = 0;
  if(icam>1)
    n0 = sum(data.wfs(1:icam-1).nssp/2);
  return 1+n0:n0+data.wfs(icam).nssp/2;
}
