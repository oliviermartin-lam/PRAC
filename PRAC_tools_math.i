 ...
 */
{
  ...
  if(is_void(listWfsType)) listWfsType = data.wfs.type;
  ...
  modes_garder1 = sum((data.wfs(others(data.its)).type==1)*(data.wfs(others(data.its)).nssp)) +
    sum((data.wfs(others(data.its)).type==2)*(data.wfs(others(data.its)).nssp-2)) +
    sum((data.wfs(others(data.its)).type==3)*(2));
  ...
  nmodes = numberof(eigenval(norange(data.its))) - modes_garder;
 ...
 */
{
  pixarc = array(1., sum(data.wfs.nssp));
  for(i=1; i<=data.nwfs; i++) {
    pixarc(slrange(i)) = data.wfs(i).pixarc;
  }
  return pixarc;
}
func identite(n)
 ...
 */
{
  ...
  mt2/= pixarc(norange(data.its))(-,);
  mt2*= pixarc(slrange(data.its));
